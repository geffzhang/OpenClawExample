...
在 OpenSandbox 沙箱实例中启动 [OpenClaw](https://github.com/openclaw/openclaw) Gateway，并暴露 HTTP 访问端点。这个 .NET 示例会轮询 Gateway，直到返回 HTTP 200，然后打印可访问地址。
...
在示例目录安装 OpenSandbox .NET SDK：

```shell
cd examples/openclaw
dotnet add package Alibaba.OpenSandbox --prerelease
```
